...
export function ExpensesView() {
  ...
  const saveExpense = () => {
    try {
      ...
      setExpenses((current) => [{ category: expense.category, label: expense.description, location: normalizedLocation || undefined, meta, value: expense.formattedAmount, amount: expense.amount, tone }, ...current]);

      if (normalizedLocation && user?.uid) {
        const nextLocations = [normalizedLocation, ...savedLocations.filter((item) => item.localeCompare(normalizedLocation, undefined, { sensitivity: "accent" }) !== 0)].slice(0, 12);
      ...
      toast.success("Gasto registrado", { description: normalizedLocation ? `${expense.description} · ${normalizedLocation}` : `${expense.description} · ${expense.formattedAmount}` });
      closeRegister();
    } catch (error) {
